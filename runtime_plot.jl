...
# ╔═╡ b14bd5c0-9e67-11ec-0e03-094f0fdd8d50
using CairoMakie

# ╔═╡ fecf4a6a-f800-43ac-b205-cbcba1b74bb9
Length = 0:12

# ╔═╡ 295c82fb-d0a7-40e3-9fb0-04ba39ad8d99
Time = [81.464, 77.248, 397.3, 682.795, 691.044, 1008.967, 1000.406, 1351.168, 1010.636, 1275.762, 1298.186, 1594.876, 1262.6]./60 #convert to mins

# ╔═╡ 67de5b5e-c3c7-45ba-a235-0b4d5929acc9
begin
	fig = Figure()
	ax = Axis(fig[1,1],
		xlabel="Length of Walk",
		ylabel="Time(min)",
		xticks=Length,
		title="Computing time for different length of walk kernel")
	scatter!(ax, Length, Time)
	lines!(ax, Length, Time)

	text!("OS: Windows 10 Home\nVersion: 21H2\nOS build: 19044.1566\nProcessor: Intel(R) Core(TM) i7-10700K CPU @3.80GHz\nCore Use: 4 Cores\nJulia Version: Julia 1.7.2", position = (0, 20), textsize = 12)
	fig
	# save("runtime.pdf", fig)
end
